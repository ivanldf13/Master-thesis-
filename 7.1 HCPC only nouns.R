...
fviz_ca_biplot(CA.NOUNS, select.row = list(contrib = 20),
               select.col = list(contrib = 20), repel = TRUE, title = "Figure 10: Top 20 contributors") +
  theme_minimal()
...
factor_map_plot <- plot(HCPC.NOUNS, 
                        choice = "map", 
                        draw.tree = FALSE,
                        title = "Figure 11: Factor map with the five clusters")
...
fviz_dend(HCPC.NOUNS, 
          ...
          type = "rectangle", 
          repel = TRUE,
          main = "Figure 9: Cluster Dendogram"
)
